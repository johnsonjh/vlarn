...
int ReadIniFile(void) {
  ...
  fscanf(fp, "%d %d %d %d %s\n", &LarnWindowLeft, &LarnWindowTop,
         &LarnWindowWidth, &LarnWindowHeight, String);
 ...
 */
static BOOL CALLBACK AboutDialogProc(HWND hwnd, UINT msg, WPARAM wparam,
                                     LPARAM lparam) {
  RECT WRect;
  RECT DRect;
  int WindowWidth;
...
void DoAbout(void) {
  DialogBox(main_instance, MAKEINTRESOURCE(IDD_ABOUT), frame_window_handle,
            AboutDialogProc);
}
...
static void CalcMinWindowSize(HDC DC) {
  ...
  MinWindowWidth = WINDOW_WIDTH * CharWidth + 2 * GetSystemMetrics(SM_CXFRAME);
  MinWindowHeight = WINDOW_HEIGHT * CharHeight + 2 * SEPARATOR_HEIGHT +
  ...
  SetWindowPos(frame_window_handle, NULL, 0, 0, LarnWindowWidth,
               LarnWindowHeight, SWP_NOMOVE | SWP_NOZORDER);
  ...
  for (i = 0; i < 17; i++) {
    ...
    if ((Repaint) || (IsSet != WasSet)) {
      if (IsSet)
        ...
        TextOut(DC, EffectsLeft, EffectsTop + i * CharHeight, "          ", 10);
    }
...
static void PaintMap(HDC DC) {
  ...
  for (x = MapTileLeft; x < mx; x++) {
    ...
    for (y = MapTileTop; y < my; y++) {
      ...
      BitBlt(DC, MapLeft + sx * TileWidth, MapTop + sy * TileHeight, TileWidth,
             TileHeight, TileDC, TileX, TileY, SRCCOPY);
  ...
  if ((sx >= 0) && (sx < MapTileWidth) && (sy >= 0) && (sy < MapTileHeight)) {
    ...
    TileY = (TileId / 16) * TileHeight;
    BitBlt(DC, MapLeft + sx * TileWidth, MapTop + sy * TileHeight, TileWidth,
           TileHeight, TileDC, TileX, TileY, SRCAND);

    TileId = TILE_CURSOR2;
    TileX = (TileId % 16) * TileWidth;
    TileY = (TileId / 16) * TileHeight;
    BitBlt(DC, MapLeft + sx * TileWidth, MapTop + sy * TileHeight, TileWidth,
           TileHeight, TileDC, TileX, TileY, SRCPAINT);
  }
}
...
static void PaintTextWindow(HDC DC) {
  ...
  if (CurrentDisplayMode == DISPLAY_TEXT) {
    if (ShowTextBorder) {
      ...
      RoundRect(DC, TLeft - 8, TTop - 8, TLeft + TWidth + 9, TTop + THeight + 9,
                16, 16);
  ...
  for (y = 0; y < MaxLine; y++) {
    ...
    while (sx < LINE_LENGTH) {
      ...
      TextOut(DC, TLeft + sx * CharWidth, TTop + y * CharHeight, Text[y] + sx,
              ex - sx);
...
static void Resize(HWND hwnd) {
  ...
  //
  if (CaretActive) {
    CreateCaret(frame_window_handle, NULL, 2, CharHeight);
...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  case WM_INITMENU: {
    ...
    CheckMenuItem(hMenu, IDM_DISPLAY_BEEP,
                  (nobeep) ? MF_UNCHECKED : MF_CHECKED);
    break;
  }
  ...
  case WM_COMMAND: {
    ...
    case IDM_DISPLAY_FONT: {
      ...
      cf.iPointSize = 0;
      cf.Flags =
          CF_SCREENFONTS | CF_FORCEFONTEXIST | CF_EFFECTS | CF_FIXEDPITCHONLY;
      cf.rgbColors = 0;
      cf.lCustData = 0;
      cf.lpfnHook = 0;
      cf.lpTemplateName = 0;
  ...
  case WM_KEYDOWN: {
    ...
    if (Found)
      ...
      /* check run key */
      if ((wparam == RunKeyMap.VirtKey) && (RunKeyMap.ModKey == ModKey))
        Runkey = 1;
    }
  ...
  case WM_CHAR: {
    ...
    if (Found)
      ...
      /* check run key */
      if ((chCharCode == RunKeyMap.VirtKey) && (RunKeyMap.ModKey == M_ASCII))
        Runkey = 1;
    }
...
static void calculate_palette(HBITMAP HSrcBmp) {
  ...
  for (y = 0; y < SrcBitmap.bmHeight; y++) {
    for (x = 0; x < SrcBitmap.bmWidth; x++) {
      SrcAddr = ((unsigned char *)SrcBitmap.bmBits) +
                SrcBitmap.bmWidthBytes * y + x * 3;
...
int init_app(HINSTANCE hinstance) {
  ...
  if (ReadIniFile()) {
    ...
    if (LarnWindowMaximized) {
      ...
      SetWindowPos(frame_window_handle, NULL, LarnWindowLeft, LarnWindowTop,
  ...
  if (GetDeviceCaps(frame_dc, BITSPIXEL) <= 8) {
    ...
    //
    TmpBitmap =
        LoadImage(hinstance, TileBMName, IMAGE_BITMAP, 0, 0,
    ...
    //
    TileBitmap = CreateDIBitmap(frame_dc, (LPBITMAPINFOHEADER)&bi, CBM_INIT,
                                lpDIBBits, (LPBITMAPINFO)&bi, DIB_RGB_COLORS);
    ...
    //
    TileBitmap = LoadImage(hinstance, TileBMName, IMAGE_BITMAP, 0, 0,
...
void Printc(char c) {
  ...
  default:
    ...
    TextOut(frame_dc, TLeft + (CursorX - 1) * CharWidth,
            TTop + (CursorY - 1) * CharHeight, &c, 1);
...
void ClearToEOL(void) {
  ...
  TextOut(frame_dc, TLeft + (CursorX - 1) * CharWidth,
          TTop + (CursorY - 1) * CharHeight, &(Text[CursorY - 1][CursorX - 1]),
...
void ClearToEOPage(int x, int y) {
  ...
  TextOut(frame_dc, TLeft + (x - 1) * CharWidth, TTop + (y - 1) * CharHeight,
  ...
  for (ty = y + 1; ty <= MaxLine; ty++) {
    ...
    TextOut(frame_dc, TLeft, TTop + (ty - 1) * CharHeight, Text[ty - 1],
            LINE_LENGTH);
  }
}
...
void show1cell(int x, int y) {
  ...
  BitBlt(frame_dc, MapLeft + sx * TileWidth, MapTop + sy * TileHeight,
         TileWidth, TileHeight, TileDC, TileX, TileY, SRCCOPY);
}
...
void showplayer(void) {
  ...
  if (scroll)
    ...
    if ((sx >= 0) && (sx < MapTileWidth) && (sy >= 0) && (sy < MapTileHeight)) {
      ...
      TileY = (TileId / 16) * TileHeight;
      BitBlt(frame_dc, MapLeft + sx * TileWidth, MapTop + sy * TileHeight,
             TileWidth, TileHeight, TileDC, TileX, TileY, SRCAND);

      TileId = TILE_CURSOR2;
      TileX = (TileId % 16) * TileWidth;
      TileY = (TileId / 16) * TileHeight;
      BitBlt(frame_dc, MapLeft + sx * TileWidth, MapTop + sy * TileHeight,
             TileWidth, TileHeight, TileDC, TileX, TileY, SRCPAINT);
 ...
 * FUNCTION: drawscreen
 */
void drawscreen(void) { InvalidateRect(frame_window_handle, NULL, 1); }
 ...
 *   0 if the message was handled by this procedure.
 */
static BOOL CALLBACK NameDialogProc(HWND hwnd, UINT msg, WPARAM wparam,
                                    LPARAM lparam) {
  int len;
  RECT WRect;
  RECT DRect;
  ...
  case WM_COMMAND: {
    ...
    case IDOK:
      ...
      //
      len = (WORD)SendDlgItemMessage(hwnd, IDC_EDIT_NAME, EM_LINELENGTH,
                                     (WPARAM)0, (LPARAM)0);
      ...
      *((LPWORD)UserName) = (WORD)len;
      ...
      SendDlgItemMessage(hwnd, IDC_EDIT_NAME, EM_GETLINE,
                         (WPARAM)0, /* line 0 */
                         (LPARAM)UserName);
...
void GetUser(char *username, int *uid) {
  ...
  *uid = -1;
  ...
  if (username[0] == 0) {
    ...
    //
    result = DialogBox(main_instance, MAKEINTRESOURCE(IDD_DIALOG1),
                       frame_window_handle, NameDialogProc);
