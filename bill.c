...
letter1() {
  ...
  lprcat("From: lrs@vlarn.dnd (Vlarn Revenue Service)\n");
...
letter3() {
  ...
  lprintf("You %s!  With several attempts I have yet to endure the",
          sex ? "Bastard" : "Bitch");
...
letter4() {
  ...
  lprcat("From: mainair@vlarn.dnd (Duke of VLarnty)\n");
...
letter5() {
  ...
  lprcat("From: stmarys@vlarn.dnd (St. Mary's Children's Home)\n");
...
letter6() {
  ...
  lprcat("From: nds@vlarn.dnd (National Dianthroritis Society of VLarn)\n");
  lprcat("Subject: hope\n");
...
mailbill() {
  ...
  if (fork() == 0) {
    ...
    for (i = 0; i < sizeof(pfn) / sizeof(int (*)()); i++)
      if (pfn[i]()) {
        sleep(20);
        sprintf(buf, "/bin/mail %s < %s", loginname, mail600);
        system(buf);
        unlink(mail600);
      }
    exit(0);
